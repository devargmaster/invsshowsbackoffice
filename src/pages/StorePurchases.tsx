...
// transferProofUrl es relativo (ej: /uploads/store-purchase-proofs/x.png),
...
function productTitle(purchase: any): string {
  return purchase.addon?.name ?? '—';
}

export function StorePurchases() {
  ...
  const load = async (showLoader = true) => {
    ...
    try {
      ...
      const data = await apiClient.get<any[]>(`/store-purchases?${params.toString()}`);
      setPurchases(data);
      ...
      if (showLoader) alert('Error al cargar compras de la Tienda');
    } finally {
  ...
  const handleValidate = async (approve: boolean) => {
    ...
    setProcessing(true);
    try {
      await apiClient.patch(`/store-purchases/${selected.id}/validate-transfer`, {
        approve,
  ...
  if (loading) return <div style={{ color: 'var(--color-text-muted)' }}>Cargando compras de la Tienda...</div>;

  return (
    <div>
      <h1 style={{ marginTop: 0, marginBottom: 8, fontSize: 28 }}>Compras de la Tienda</h1>
      <p style={{ color: 'var(--color-text-muted)', marginBottom: 32 }}>
        Validá los comprobantes de compras de productos sueltos (sin entrada) para habilitarlas.
      </p>
      ...
      {purchases.length === 0 ? (
        <div style={{ color: 'var(--color-text-muted)', textAlign: 'center', marginTop: 40 }}>
          No hay compras de la Tienda por transferencia en este estado.
        </div>
      ) : (
          ...
          <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <th style={{ padding: '16px 24px', color: 'var(--color-text-muted)', fontWeight: 500 }}>Producto</th>
                <th style={{ padding: '16px 24px', color: 'var(--color-text-muted)', fontWeight: 500 }}>Cantidad</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '16px 24px', color: 'var(--color-text-secondary)' }}>
                      {productTitle(p)}{p.variant ? ` — ${p.variant.label}` : ''}
                    </td>
                    <td style={{ padding: '16px 24px', color: 'var(--color-text-secondary)' }}>{p.quantity}</td>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="glass" style={{ width: 480, padding: 32, borderRadius: 24, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 4 }}>Producto</div>
              <div style={{ fontSize: 15, fontWeight: 600 }}>{productTitle(selected)}{selected.variant ? ` — ${selected.variant.label}` : ''}</div>
              <div style={{ fontSize: 13, color: 'var(--color-text-muted)', marginTop: 2 }}>Cantidad: {selected.quantity}</div>
            </div>
