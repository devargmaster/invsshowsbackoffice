...
export function Orders() {
  ...
  if (loading) return <div style={{ color: 'var(--color-text-muted)' }}>Cargando órdenes...</div>;

  return (
    <div>
      <h1 style={{ marginTop: 0, marginBottom: 8, fontSize: 28 }}>Pagos por Transferencia</h1>
      <p style={{ color: 'var(--color-text-muted)', marginBottom: 32 }}>Validá los comprobantes para activar las entradas.</p>
          ...
          <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                ...
                <th style={{ padding: '16px 24px', color: 'var(--color-text-muted)', fontWeight: 500 }}>Entradas</th>
                <th style={{ padding: '16px 24px', color: 'var(--color-text-muted)', fontWeight: 500 }}>Total</th>
              ...
              {orders.map(o => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '16px 24px', color: 'var(--color-text-secondary)' }}>{o.event?.title}</td>
                    <td style={{ padding: '16px 24px', color: 'var(--color-text-secondary)' }}>{o.tickets?.length ?? 0}</td>
      ...
      {selected && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div className="glass" style={{ width: 480, padding: 32, borderRadius: 24, maxHeight: '85vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 4 }}>Entradas ({selected.tickets?.length ?? 0})</div>
              <div style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>{selected.tickets?.length ?? 0} unidad(es)</div>
            </div>

            {selected.addons?.length > 0 && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 4 }}>Adicionales</div>
                {selected.addons.map((a: any) => (
                  <div key={a.id} style={{ fontSize: 14, color: 'var(--color-text-secondary)' }}>
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: 'var(--color-text-muted)', fontSize: 13, marginBottom: 4 }}>Total</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--color-accent)' }}>{formatMoney(selected.totalCents, selected.currency)}</div>
            </div>
